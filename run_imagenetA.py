...
batch_size = 128
naes = torchvision.datasets.ImageFolder(root="./imagenet-a/", transform=test_transform)
nae_loader = torch.utils.data.DataLoader(naes, batch_size=batch_size, shuffle=True)
...
output_dir = 'Results/'
output_filename = 'ImageNetA_Perf_on_Resnext101_' + now.strftime("%m_%d_%Y-%H_%M_%S") + '.txt'
...
def get_net_results():
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(nae_loader):
            ...
            # print(data.shape, target.shape)     # [128, 3, 224, 224] [128]

            # Run each network and store output in 'out'
            out = torch.empty(
                (len(MODELS), target.shape[0], 200))  # Shape [M, N, 200] where M = num models, and N = batch size
            for i, net in enumerate(networks):
                out[i, :, :] = net(data)[:,indices_in_1k]

            predictions = vote(out, voting_heuristic)  # Returns predictions, with shape [N] (one prediction per image)
            if cuda:
                predictions = predictions.cuda()
            num_correct += torch.sum(predictions == target).item()  # Item() pulls the integer out of the tensor

            # Original code:
            # output = net(data)[:,indices_in_1k]
            # # print("output shape:", output.shape)    # [128, 200]
            #
            # # accuracy
            # pred = output.data.max(1)[1]
            # # print("pred shape:", pred.shape)        # [128]
            # num_correct += pred.eq(target.data).sum().item()
            #
            # confidence.extend(to_np(F.softmax(output, dim=1).max(1)[0]).squeeze().tolist())
            # pred = output.data.max(1)[1]
            # correct.extend(pred.eq(target).to('cpu').numpy().squeeze().tolist())
            print("Batch:", batch_idx, "has", num_correct, "correct")

    return num_correct / len(nae_loader.dataset)


acc = get_net_results()

write('ImageNet-A Accuracy (%):' + str(round(100*acc, 4)))